# Hướng Dẫn Deploy (Production an toàn + Demo có seed)

Ứng dụng hỗ trợ 2 mode:
- Production an toàn: không seed dữ liệu khi build.
- Demo/portfolio: có thể seed thủ công hoặc dùng build command demo.
...
- **Bước 3:** Ở phần **Build and Output Settings**, ghi đè **Build Command** thành lệnh sau:
  ...
  ```
  *(Lệnh này migrate DB + generate Prisma client + build, KHÔNG seed dữ liệu mẫu.)*
...
Neu muon deploy ban demo co du lieu mau, dung:
```bash
npm run vercel-build:demo
```
