...
import { parseMilestones, parseProgressUpdates, parseRating } from "./parser";
...
describe("progress parser", () => {
  it("parses valid milestones and filters invalid entries", () => {
    ...
    expect(milestones).toEqual([{ id: "m1", title: "Kickoff", createdAt: "2026-03-21T00:00:00.000Z" }]);
  });
  ...
  it("parses valid progress updates and filters invalid entries", () => {
    ...
    expect(updates).toEqual([{ id: "u1", content: "Done A", createdAt: "2026-03-21T00:00:00.000Z" }]);
  });
